...
//    [People bg_update:bg_tablename version:1];
    ...
    //顺便取第一个对象数据测试
        if(i==1){
            ...
            NSData *imageData = UIImageJPEGRepresentation(lastP.image, 1);
 ...
 */
- (IBAction)deleteAction:(id)sender{
    NSString* where = [NSString stringWithFormat:@"where %@=%@",bg_sqlKey(bg_primaryKey),@(3)];
    [People bg_delete:bg_tablename where:where];
}
/**
 更新
...
//自定义测试
- (IBAction)customTestModlue:(id)sender {
    
    customTestViewController *testVC = [[customTestViewController alloc]init];
